...
struct API {
    ...
    static let urlBaseMovies = "https://api.themoviedb.org/3/movie/now_playing"
    ...
    static func getMovies(page: Int) -> Observable<ResultMovies> {
        guard let url = URL(string: "\(urlBaseMovies)?api_key=\(apiKey)&&language=\(language)&page=\(page)")
